 ...
 * @LastEditTime: 2023-03-13 23:20:33
 * @Description:
...
将每次从环境中采样得到的四元组数据（状态 $s_t$、动作 $a_t$、奖励 $r_t$、下一状态 $s_{t+1}$ ），存储到**回放缓冲区**中，训练 Q 网络的时候再从回放缓冲区中随机采样若干数据来进行训练。这样每次的四元组数据称为一笔数据。这个技巧就叫做**经验回放**（experience replay）。
