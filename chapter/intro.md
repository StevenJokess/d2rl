 ...
 * @LastEditTime: 2023-03-08 22:42:19
 * @Description:
...
- 奖励函数（Reward Functions）：实际每个状态或动作有多好。
- 价值(Value)、价值函数(Value Functions)：预测每个状态或动作有多好；优势函数（Advantage Functions）：预测该状态相对其他状态有多好。
- 策略(Policy)、策略函数(Policy Functions)：行动的函数
- 模型（Model）：智能体对环境的表征（representation）。
...
**价值(Value)**或效用: 价值 $V$ 是对未来折扣累积奖励的预测值，是估计该状态的期望回报（即从这个状态出发的未来累积奖励的期望）[18]。
...
策略(Policy)：策略 $\pi$ 是状态到动作的函数，输入是状态，而输出是智能体在各个状态下应该采取的动作。


具体策略：之后会以多臂老虎机N-Armed Bandit (N = 10)问题为例介绍几种策略，见[下一章：多臂老虎机](MAB.md)
...
- 被动学习中智能体的策略是固定的，任务是学习状态（或状态 - 行为配对）的价值，也可能涉及到学习环境模型。
...
- 状态转移概率是指当前状态到另一特定状态的概率，即 $p_{s s^{\prime}}^a=p\left(s_{t+1}=s^{\prime} \mid s_t=s, a_t=a\right)$ 具体见[马尔可夫过程章节](MDP.md)
- 奖励函数是指我们在当前状态采取了某个动作，可以得到多大的奖励，即 $R(s, a)=\mathbb{E}\left[r_{t+1} \mid s_t=s, a_t=a\right]$[27]
