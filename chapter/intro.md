 ...
 * @LastEditTime: 2023-03-11 00:52:00
 * @Description:
...
|      | 监督学习  | 非监督学习  | 强化学习 |
| ---- | ---------------- | ------------- | ------ |
| 数据 | 一次给定的supervisor提供正确且严格的标签 | 没有标签，       | 没有标签、supervisor，属于semi supervised learning，时间序列数据（sequential data）是在智能体与环境交互的过程中得到的。如果智能体不采取某个决策动作，那么该动作对应的数据就永远无法被观测到，所以当前智能体的训练数据来自之前智能体的决策结果。
...
> 一个完整的智能体还需要有传感器（Actuators）去感知和观测环境，即接受输入为观察（定义见后），和执行器（Actuators）去作用于外部环境，即将动作（定义见后）的输出传递给环境。[25]
>
...
- 回合制任务（episodic/terminating task）或阶段性任务：对于回合制任务，有明确的开始状态和终止状态(Terminal State)[21]。当到达终止状态时，一个智能体和环境的交互过程就结束了，这一轮交互的过程称为一个回合（Episode）或试验（Trial）。很多任务都是回合制的，如围棋等棋类、回合游戏。
- 连续性任务（continuing/non-terminating[32] task）：对于连续性任务，没有明确的开始结束条件。即 $T=\infty$

#### 任务指标

性能度量（Performance Measure）：用于描述任务中智能体行为成功的客观指标
...
> 奖励函数（Reward Functions）：描述当前状态到下一个状态的实际奖励。

- 价值（Value）、价值函数（Value Functions）：描述当前状态到下一个状态的奖励预测值；优势函数（Advantage Functions）：去描述预测该状态相对其他状态有多好。
- 策略（Policy）、策略函数（Policy Functions）：行动的函数。

- 模型（Model）：智能体对环境的表征（representation），已知的状态转移函数、状态转移概率和由状态和动作为输入的奖励函数。
...
策略(Policy)：强化学习通过学习来改进策略来最大化总奖励。

根据策略固定与否分为不同的学习模式：

- 被动学习中智能体的策略是固定的，任务是学习状态（或状态 - 行为配对）的价值，也可能涉及到学习环境模型。这种固定的策略叫做确定性策略（deterministic policy） [21]，通常智能体直接采取最有可能（最大概率）的动作
- 主动学习主要涉及的问题是探索：智能体必须尽可能多地体验其环境，以便学习如何表现。

具体策略：之后会以多臂老虎机N-Armed Bandit (N = 10)问题为例介绍几种具体策略，见[下一章：多臂老虎机](MAB.md)

##### 策略函数(Policy Functions)

策略函数(Policy Functions)是 观察（Observation） 到 行动（Action）的一个映射   或 全观察的状态（fully observated State) [23] 到 Action 的一个映射  。以雅达利游戏为例子，策略函数的输入就是游戏的任一帧，它的输出决定智能体向左移动或者向右移动。

##### 策略概率（(Policy Probability）

策略概率是指该状态下$\pi_\theta\left(\mathbf{a}_t \mid \mathbf{o}_i\right)$ $\pi_\theta\left(\mathbf{a}_t \mid \mathbf{s}_t\right)$

- 用策略概率表达即$\mu(s)= a^* =\underset{a}{\arg \max } \pi(a \mid s)$。
...
- 状态转移函数（State Transition Function）定义了当前状态下执行某个动作后可能转移到的下一个状态集合。状态转移函数的作用是确定MDP中的动作空间。
- 状态转移概率（State Transition Probability）是指当前状态到下一个特定状态的概率，即 $p_{s s^{\prime}}=p\left(s_{t+1}=s^{\prime} \mid s_t=s \right)$。在[马尔可夫过程 MDP](MDP.md)，下一步的状态取决于当前的状态以及当前采取的动作。故可以将状态转移函数表示为：$T(s,a,s') = P(s'|s,a)$其中，s是当前状态，a是采取的行动，s'是下一个状态，T(s,a,s')表示在状态s下采取行动a转移到状态s'的概率。状态转移概率是用于计算值函数或策略的期望收益。

描述奖励的概念：

- 奖励函数是指我们在当前状态采*取了某个动作*，可以得到多大的奖励，即 $R(s, a)=\mathbb{E}\left[r_{t+1} \mid s_t=s, a_t=a\right]$[27]
