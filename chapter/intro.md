 ...
 * @LastEditTime: 2023-03-11 03:43:00
 * @Description:
...
策略（Policy）：是观察（Observation）到 行动（Action）的一个映射   或 全观察的状态（fully observated State) [23] 到 Action 的一个映射。 它对应于心理学中所谓的一组刺激-反应规则或关联。强化学习通过学习来改进策略来最大化总奖励。
...
策略函数(Policy Functions)是   。以雅达利游戏为例子，策略函数的输入就是游戏的任一帧，它的输出决定智能体向左移动或者向右移动。
